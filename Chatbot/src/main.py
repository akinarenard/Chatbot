...
def on_agent_event_callback(event, uuid, name, event_data, my_data):
    ...
    assert isinstance(agent_object, Chatbot)
...
def Command_input_callback(iop_type, name, value_type, value, my_data):
    ...
    # add code here if needed
    agent_object.interpretCommand(value)


def data_input_callback(iop_type, name, value_type, value, my_data):
    agent_object = my_data
    assert isinstance(agent_object, Chatbot)
    agent_object.dataI = json.dumps(value, indent = 4) 
    
    # add code here if needed
    agent_object.updateData(value)
...
if __name__ == "__main__":
    ...
    igs.input_create("data", igs.STRING_T, None)

    igs.output_create("JSON", igs.STRING_T, None)
    igs.output_create("reducedData", igs.STRING_T, None)
