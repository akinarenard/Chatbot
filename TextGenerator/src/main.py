...
def JSON_input_callback(iop_type, name, value_type, value, my_data):
    ...
    if "error" in agent_object.JSONI:
        TextGenerator.set_ErrorO()
    if "products" in agent_object.JSONI:
        TextGenerator.set_ValidO() 
...
if __name__ == "__main__":
    ...
    igs.input_create("JSON", igs.STRING_T, None)

    igs.output_create("Text", igs.STRING_T, None)
    igs.output_create("Valid", igs.IMPULSION_T, None)
    igs.output_create("Error", igs.IMPULSION_T, None)

    igs.observe_input("JSON", JSON_input_callback, agent)
